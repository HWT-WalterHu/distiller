...
#    conf.test_set = ["Business_small_95to5", "XCH_out2_95to5"]
    ...
    conf.teacher_model_path = '/media/hwt/492c0a80-02ce-45ac-9297-63ce30dfdd81/home/minivision/Project/pytorch_mo' \
                              'dels/FaceRecognition/best_select_model/2019-03-19-09-14/SVGArcFace-P6FT2-b0.4s40t1.1Pm' \
                              '5.3_fc_0.35_112x96_del-LAN-DHUA-Crop-Kd1-2-TYLG-NL-GE8_FaceNetOrg20-d512/2019-03-15-21-58_' \
                              'SVGArcFace-P6FT2-b0.4s40t1.1Pm5.3_fc_0.35_112x96_del-LAN-DHUA-Crop-Kd1-2-TYLG-NL-GE8_FaceNetOr' \
                              'g20-d512_model_iter-115000_Bus-0.7477.pth'

    teacher_file_name = conf.teacher_model_path.split('/')[-1]
    _, _, t_train_subject, t_loss_type, _, _, _ = \
        get_training_param_from_filename(teacher_file_name)
    # set ArcFace loss param  bias, scale_value
    sphere_param_part = t_train_subject.split('-')[-1]
    b_index = sphere_param_part.find('b')
    s_index = sphere_param_part.find('s')
    bias = float(sphere_param_part[b_index + 1:s_index])
    scale_value = float(sphere_param_part[s_index + 1:])
