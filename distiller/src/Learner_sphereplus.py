...
from src.models.model import sphere_plusLoss
...
class face_learner(object):
    def __init__(self, conf, ):
        ...
        self.head_sp = sphere_plusLoss(embedding_size=conf.embedding_size, classnum=self.class_num,
                                       type = conf.sp_type, alpha=conf.alpha).to(conf.device)
        ...
        if conf.fintune_model_path == None:
            ...
            if opt_prefix != "":
                ...
                if os.path.exists(head_sp_path):
                    self.head_sp.load_state_dict(torch.load(head_sp_path))
    ...
    def save_state(self, conf, accuracy, extra=None, model_only=False):
        ...
        if not model_only:
            ...
            torch.save(
                self.head_sp.state_dict(), save_path /
                ('{}_{}_headsp_iter-{}_{}.pth'.format(get_time(), extra, self.step, acc_str)))
    ...
    def train(self, conf,):
        ...
        for e in range(self.start_epoch, conf.epochs):
            ...
            for imgs, labels in tqdm(iter(self.loader)):
                ...
                running_inter_loss += inter_loss.item() / conf.batch_size

                self.optimizer.step()

                if self.step % self.board_loss_every == 0 and self.step != 0:
                    loss_board = running_loss / self.board_loss_every
                    inter_loss_board = running_inter_loss / self.board_loss_every
